...
const UBIDBUDDY_TRANSPORT_ERROR_CODES = new Set([
...
export const buildSendFailureError = (error: unknown, message: string): AgentStreamErrorInfo => {
  ...
  if (isBackendHttpError(error) && UBIDBUDDY_TRANSPORT_ERROR_CODES.has(error.code)) {
    return {
      message,
      code: error.code,
      ownership: 'ubidbuddy',
  ...
  if (busyError) {
    return {
      message,
      code: 'UBIDBUDDY_CONVERSATION_BUSY',
      ownership: 'ubidbuddy',
      detail: message,
      retryable: false,
      feedback_recommended: false,
