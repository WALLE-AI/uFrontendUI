 ...
 * the user picked inside uBidBuddy. A German desktop running uBidBuddy in English used
 * to render `0,42 $` and `17.8.2025` next to English labels, and the same app on
 * a `zh-HK` desktop rendered a locale uBidBuddy does not even ship.
 *
...
/**
 * Reduce any language hint to a locale uBidBuddy actually ships, so formatting is
