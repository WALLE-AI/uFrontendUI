...
/**
 * Minimal stand-in for Electron's `OpenDialogOptions` shape (only the fields
 * `ShowOpenOptions` below actually reuses) — this standalone build has no
...
// Electron-native window/DevTools/CDP/GPU/start-on-boot/log-stream operations
// have no meaning outside a desktop shell and no backend handler in a plain
// HTTP/WS deployment — stubbed as no-ops (see project README for how to wire
// real implementations back in if you ever embed this in a desktop shell).
export const application = {
  restart: stubProvider<IAppRestartResult, void>('application.restart', {
    restarted: false,
    manualRestartRequired: true,
  }),
  openDevTools: stubProvider<boolean, void>('application.openDevTools', false),
  isDevToolsOpened: stubProvider<boolean, void>('application.isDevToolsOpened', false),
  ...
  getPath: stubProvider<string, { name: 'desktop' | 'home' | 'downloads' }>('application.getPath', ''),
  updateSystemInfo: stubProvider<void, { cacheDir: string; workDir: string; logDir?: string }>(
    'application.updateSystemInfo',
    undefined
  ),
  getZoomFactor: stubProvider<number, void>('application.getZoomFactor', 1),
  setZoomFactor: stubProvider<number, { factor: number }>('application.setZoomFactor', 1),
  getCdpStatus: stubProvider<IBridgeResponse<ICdpStatus>, void>('application.getCdpStatus', {
    success: true,
    data: { enabled: false, port: null, startupEnabled: false, configEnabled: false, isDevMode: false },
  }),
  updateCdpConfig: stubProvider<IBridgeResponse<ICdpConfig>, Partial<ICdpConfig>>('application.updateCdpConfig', {
    success: false,
  }),
  clearBrowserData: stubProvider<IBridgeResponse<void>, void>('application.clearBrowserData', { success: false }),
  reportBrowserWebContentsId: stubProvider<IBridgeResponse<void>, { webContentsId: number }>(
    'application.reportBrowserWebContentsId',
    { success: false }
  ),
  getStartOnBootStatus: stubProvider<IBridgeResponse<IStartOnBootStatus>, void>('application.getStartOnBootStatus', {
    success: true,
    data: { supported: false, enabled: false, isPackaged: false, platform: 'web' },
  }),
  setStartOnBoot: stubProvider<IBridgeResponse<IStartOnBootStatus>, { enabled: boolean }>(
    'application.setStartOnBoot',
    { success: false }
  ),
  getGpuStatus: stubProvider<IBridgeResponse<IGpuStatus>, void>('application.getGpuStatus', {
    success: true,
    data: { userOverride: null, autoDisabled: false, crashCount: 0, lastCrashAt: null },
  }),
  setGpuOverride: stubProvider<IBridgeResponse<IGpuStatus>, { override: IGpuOverride | null }>(
    'application.setGpuOverride',
    { success: false }
  ),
  writeRendererLog: stubProvider<void, IRendererLogEntry>('application.writeRendererLog', undefined),
  logStream: stubEmitter<{ level: 'log' | 'warn' | 'error'; tag: string; message: string; data?: unknown }>(
    'application.logStream'
  ),
  devToolsStateChanged: stubEmitter<{ isOpen: boolean }>('application.devToolsStateChanged'),
};

// ---------------------------------------------------------------------------
// Update — Electron-native auto-updater, no-op in this standalone build
// ---------------------------------------------------------------------------

export const update = {
  open: stubEmitter<{ source?: 'menu' | 'about' | 'tray' }>('update.open'),
  check: stubProvider<IBridgeResponse<UpdateCheckResult>, UpdateCheckRequest>('update.check', { success: false }),
  consumeInstallerLastFailure: stubProvider<IBridgeResponse<InstallerLastFailureMarker | null>, void>(
    'update.consumeInstallerLastFailure',
    { success: true, data: null }
  ),
  download: stubProvider<IBridgeResponse<UpdateDownloadResult>, UpdateDownloadRequest>('update.download', {
    success: false,
  }),
  cancelDownload: stubProvider<IBridgeResponse, UpdateDownloadCancelRequest>('update.cancelDownload', {
    success: false,
  }),
  downloadProgress: stubEmitter<UpdateDownloadProgressEvent>('update.downloadProgress'),
};
...
export const autoUpdate = {
  ...
  >('autoUpdate.check', { success: false }),
  restoreDownloaded: stubProvider<IBridgeResponse<AutoUpdateReadyResult>, void>('autoUpdate.restoreDownloaded', {
    success: false,
  }),
  download: stubProvider<IBridgeResponse, void>('autoUpdate.download', { success: false }),
  cancelDownload: stubProvider<IBridgeResponse, void>('autoUpdate.cancelDownload', { success: false }),
  quitAndInstall: stubProvider<void, void>('autoUpdate.quitAndInstall', undefined),
  status: stubEmitter<AutoUpdateStatus>('autoUpdate.status'),
};
...
// ---------------------------------------------------------------------------
// Deep Link — Electron protocol handler, no-op in this standalone build
// ---------------------------------------------------------------------------

export const deepLink = {
  received: stubEmitter<{
    action: string;
...
// ---------------------------------------------------------------------------
// Window Controls — Electron-native, no-op in this standalone build
// ---------------------------------------------------------------------------

export const windowControls = {
  minimize: stubProvider<void, void>('windowControls.minimize', undefined),
  maximize: stubProvider<void, void>('windowControls.maximize', undefined),
  unmaximize: stubProvider<void, void>('windowControls.unmaximize', undefined),
  close: stubProvider<void, void>('windowControls.close', undefined),
  isMaximized: stubProvider<boolean, void>('windowControls.isMaximized', false),
  maximizedChanged: stubEmitter<{ is_maximized: boolean }>('windowControls.maximizedChanged'),
};

// ---------------------------------------------------------------------------
// Theme — Electron main process owned the resolved-theme cache for
// multi-window sync; no-op in this standalone (single-window) build.
// ---------------------------------------------------------------------------

export const theme = {
  changed: stubEmitter<Theme>('theme.changed'),
  setActive: stubProvider<void, Theme>('theme.setActive', undefined),
  requestCurrent: stubProvider<Theme | null, void>('theme.requestCurrent', null),
};
...
export const systemSettings = {
  // System tray: Electron-only feature, no-op in this standalone build.
  getCloseToTray: stubProvider<boolean, void>('systemSettings.getCloseToTray', false),
  setCloseToTray: stubProvider<void, { enabled: boolean }>('systemSettings.setCloseToTray', undefined),
  ...
  // Desktop-pet settings: Electron-only feature, no-op in this standalone build.
  getPetEnabled: stubProvider<boolean, void>('systemSettings.getPetEnabled', false),
  setPetEnabled: stubProvider<void, { enabled: boolean }>('systemSettings.setPetEnabled', undefined),
  getPetSize: stubProvider<number, void>('systemSettings.getPetSize', 1),
  setPetSize: stubProvider<void, { size: number }>('systemSettings.setPetSize', undefined),
  getPetDnd: stubProvider<boolean, void>('systemSettings.getPetDnd', false),
  setPetDnd: stubProvider<void, { dnd: boolean }>('systemSettings.setPetDnd', undefined),
  getPetConfirmEnabled: stubProvider<boolean, void>('systemSettings.getPetConfirmEnabled', false),
  setPetConfirmEnabled: stubProvider<void, { enabled: boolean }>('systemSettings.setPetConfirmEnabled', undefined),
...
// ---------------------------------------------------------------------------
// Notification — Electron-native OS Notification API, no-op in this
// standalone build (wire this up to the browser Notification API if desired).
// ---------------------------------------------------------------------------
...
export const notification = {
  show: stubProvider<void, INotificationOptions>('notification.show', undefined),
  clicked: stubEmitter<{ conversation_id?: string }>('notification.clicked'),
};
...
// ---------------------------------------------------------------------------
// WebUI — start/stop/getStatus/statusChanged managed an Electron-hosted WebUI
// server process; irrelevant here since this standalone app IS the web server
// (no-op). Credential operations route to backend /api/webui/* unchanged.
// ---------------------------------------------------------------------------
...
export const webui = {
  getStatus: stubProvider<IWebUIStatus, void>('webui.getStatus', {
    running: true,
    port: typeof window !== 'undefined' ? Number(window.location.port) || 0 : 0,
    allowRemote: true,
    localUrl: typeof window !== 'undefined' ? window.location.origin : '',
    adminUsername: '',
  }),
  start: stubProvider<IWebUIStartResult, { port?: number; allowRemote?: boolean }>('webui.start', {
    port: 0,
    allowRemote: false,
    localUrl: '',
  }),
  stop: stubProvider<void, void>('webui.stop', undefined),
  statusChanged: stubEmitter<{
    running: boolean;
    port?: number;
    localUrl?: string;
    networkUrl?: string;
