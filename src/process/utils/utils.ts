 ...
 * Release builds use ~/.ubidbuddy; dev builds use ~/.ubidbuddy-dev.
 * 获取数据目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.ubidbuddy，Dev 模式使用 ~/.ubidbuddy-dev。
 */
 ...
 * Release builds use ~/.ubidbuddy-config; dev builds use ~/.ubidbuddy-config-dev.
 * 获取配置目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.ubidbuddy-config，Dev 模式使用 ~/.ubidbuddy-config-dev。
 */
...
export async function verifyDirectoryFiles(dir1: string, dir2: string): Promise<boolean> {
  try {
    ...
    console.warn('[uBidBuddy] Error verifying directory files:', error);
    return false;
  }
}
...
export const copyFilesToDirectory = async (
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.warn(`[uBidBuddy] Source file does not exist, skipping: ${absoluteFilePath}`);
      console.warn(`[uBidBuddy] Original path: ${file}`);
      // 跳过不存在的文件，而不是抛出错误
      continue;
    }
    ...
    try {
      ...
      console.error(`[uBidBuddy] Failed to copy file from ${absoluteFilePath} to ${destPath}:`, error);
      // 继续处理其他文件，而不是完全失败
    }
